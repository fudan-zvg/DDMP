            ...
            for dbind, db in enumerate(conf.datasets_train):
                ...
                if db['anno_fmt'].lower() == 'kitti_det':
                    ...
                    for annind, annpath in enumerate(annlist):
                        ...
                        # depthpath = os.path.join(train_folder, 'depth_2', str(int(id)) + '.pkl')
        ...
        if not self.video_det:
            ...
            if not self.use_seg:
                if self.depth_channel == 3:
                    ...
                    depth = xy_to_xyz(depth)
                    # depth = cv2.imread(self.imdb[index].path_depth)
                    '''
                    with open(self.imdb[index].path_depth, 'rb') as file:
                        depth = torch.load(file)
                    '''
                else:
                    depth = cv2.imread(self.imdb[index].path_depth, cv2.IMREAD_UNCHANGED)
...
def xy_to_xyz(img_xy):
    x, y = img_xy.shape[0], img_xy.shape[1]
    axis1 = np.tile(np.arange(x, dtype='uint16')[:, np.newaxis], (1, y))
    axis2 = np.tile(np.arange(y, dtype='uint16')[np.newaxis,:], (x, 1))
    img_xy = img_xy[:, :, np.newaxis]
    axis1 = axis1[:, :, np.newaxis]
    axis2 = axis2[:, :, np.newaxis]
    img_xyz = np.concatenate([img_xy, axis2, axis1], axis=2)
    return img_xyz
